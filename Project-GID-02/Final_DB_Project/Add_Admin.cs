    ...
    public partial class Add_Admin : Form
    {
        ...
        private void bunifuButton1000_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (getFound == true)
                {
                    //SqlCommand t1_cmd = new SqlCommand("SET TRANSACTION ISOLATION LEVEL SERIALIZABLE begin tran Insert_Admin", con);
                    ...
                    da2.Fill(dt2);
                    SqlCommand getId = new SqlCommand("SELECT Id FROM Lookup WHERE Name = 'Admin' ", con);
                    ...
                    myreader1.Close();
                    SqlCommand cmd10 = new SqlCommand("SELECT Id FROM Pharmacy WHERE Location = @Name", con);
                    cmd10.Parameters.AddWithValue("Name", phar_Box.SelectedItem.ToString());
                    cmd10.ExecuteNonQuery();
                    
                    foreach (KeyValuePair<string, string> getKeys in adminCheck)
                    {
                        if (int.Parse(getKeys.Value) == int.Parse(getId.ExecuteScalar().ToString()) && int.Parse(cmd10.ExecuteScalar().ToString()) == pharId)
                        {
                            adminFound = true;
                        }
                    }
                    ...
                    if (checkPass == false && checkContact == false && checkCNIC == false && checkEmail == false && adminFound == false)
                    {
                        ...
                        cmd.Parameters.AddWithValue("@Designation", int.Parse(getId.ExecuteScalar().ToString()));
                        ...
                        cmd.Parameters.AddWithValue("@PharmacyId", int.Parse(cmd10.ExecuteScalar().ToString()));
                ...
                //if (checkLoop == 1)
                //{
                //    SqlCommand t2_cmd = new SqlCommand("commit tran Insert_Admin", con);
                //    t2_cmd.ExecuteNonQuery();
                
                //}
                //else if (checkLoop == 0)
                //{
                //    SqlCommand t2_cmd = new SqlCommand("rollback tran Insert_Admin", con);
                //    t2_cmd.ExecuteNonQuery();
                //    showEmployeeData();
                //}
                if (checkEmail == true)
        ...
        private void Add_Admin_Load(object sender, EventArgs e)
        {
            showEmployeeData();
            var con = Configuration.getInstance().getConnection();
            SqlCommand cmd4 = new SqlCommand("SELECT Location FROM Pharmacy ", con);
            SqlDataReader DR4 = cmd4.ExecuteReader();
            while (DR4.Read())
            {
                phar_Box.Items.Add(DR4[0]);
            }
            DR4.Close();
